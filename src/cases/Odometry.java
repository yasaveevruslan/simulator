...
public class Odometry implements IState{
    ...
    private boolean WhatIsElement(String name, float x, float y, float z, SmoothEnum mode)
    {
        ...
        float gyro = (float) Elements.angle;
        ...
        else
        {
            ...
            {
                coordinateX = coordinates[0] + Elements.positionX;
                coordinateY = coordinates[1] + Elements.positionY;
                coordinateZ = z + gyro;
    ...
    private boolean Drive(float x, float y, float z, SmoothEnum mode, boolean Relative)
    {
        ...
        float nowX = x - Elements.positionX;
        ...
        if (StateMachine.isFirst)
        {
            ...
            {
                position = Function.axis(x, y, Elements.positionX, Elements.positionY);
            }
            else
            {
                position = 0;
        ...
        float speedX = (float)(r * Math.cos(theta));
        float speedY = (float)(r * Math.sin(theta));
        float speedZ = Function.TransF(speedZfunc, nowZ);
